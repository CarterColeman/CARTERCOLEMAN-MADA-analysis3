...
clean_df_location3 <- here::here("data", "processed_data", "processeddata3.RDS")
...
collect_metrics(dtree_final_fit)

#Predict values and plot model fit for predicted values
library(rpart.plot)#package used to plot predicted value model
dtree_pred <- 
  dtree_final_fit %>%
  collect_predictions()
...
#Predict values and plot model fit for predicted values
dtree_Pred_final <- collect_predictions(dtree_final_fit)
  
autoplot(dtree_Pred_final)
```
...
_This shows that one of the shortest trees (depth = 1) is the best performing models (RMSE = 1.19; STE = 0.018). However, it doesn't really perm any better than the null model, making it a bad fit to the data._
